...
import { User, Mail, Phone, Shield, Clock, Zap, CheckCircle, XCircle } from 'lucide-react'
      ...
      <div className="grid grid-cols-2 gap-4 mb-5">
        <div className="surface rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>AI Mode</span>
          </div>
          <div className="flex items-center gap-2">
            {user.ai_mode_enabled
              ? <CheckCircle size={20} style={{ color: 'var(--acid)' }} />
              : <XCircle size={20} style={{ color: 'var(--ember)' }} />}
            <span className="font-display font-600" style={{ color: user.ai_mode_enabled ? 'var(--acid)' : 'var(--ember)' }}>
              {user.ai_mode_enabled ? 'Đang bật' : 'Đã tắt'}
            </span>
          </div>
        </div>
        ...
        <div className="surface rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            <Clock size={16} style={{ color: 'var(--frost)' }} />
            <span className="text-xs uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Hết hạn</span>
          </div>
          {user.access_expires_at ? (
            <div>
              <p className="font-display font-600 text-sm" style={{ color: isExpired ? 'var(--ember)' : 'var(--text)' }}>
                {format(new Date(user.access_expires_at), 'dd/MM/yyyy')}
              </p>
              {isExpired && (
                <p className="text-xs mt-0.5" style={{ color: 'var(--ember)' }}>Đã hết hạn</p>
              )}
            </div>
          ) : (
            <p className="font-display font-600 text-sm" style={{ color: 'var(--acid)' }}>Không giới hạn</p>
          )}
        </div>
      </div>
